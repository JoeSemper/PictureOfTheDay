...
class PictureOfTheDayDetailFragment : Fragment() {
    ...
    private fun renderData() {
        ...
        val header = SpannableString(args.title)
        header.setSpan(UnderlineSpan(), 0, args.title.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        tv_detail_header.text = header

        val string = SpannableString(args.description)
        string.setSpan(ForegroundColorSpan(ContextCompat.getColor(requireContext(), R.color.material_deep_teal_500)),
            0, args.description.length,
            Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        tv_detail_description.text = string
